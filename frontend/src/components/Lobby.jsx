...
export default function Lobby() {
    ...
    const [stats, setStats] = useState({ escuelas: '—', necesidades: '—', municipios: '—' });

    useEffect(() => {
        getStats()
            .then(res => setStats(res.data))
            .catch(() => {}); // silently keep the dashes if the request fails
    }, []);

    const cards = [
        { icon: school, alt: 'escuela', value: stats.escuelas,   label: 'Escuelas registradas'  },
        { icon: heart,  alt: 'corazon', value: stats.necesidades, label: 'Necesidades activas'    },
        { icon: people, alt: 'gente',   value: stats.municipios,  label: 'Municipios alcanzados'  },
    ];
            ...
            <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-10">
                ...
                <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
                    {cards.map(({ icon, alt, value, label }) => (
                        <div
                            key={label}
                            className="flex flex-col items-center rounded-[28px] border border-white/15 bg-white/10 px-6 py-8 text-center shadow-[0_18px_45px_rgba(4,120,87,0.2)] backdrop-blur-md"
                        >
                            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10">
                                <img className="h-8 w-8 object-contain" src={icon} alt={alt} />
                            </div>
                            <h2 className="text-3xl font-extrabold text-white">{value}</h2>
                            <p className="mt-2 text-sm font-medium text-emerald-50/90">{label}</p>
                        </div>
                    ))}
                </div>
            </div>
